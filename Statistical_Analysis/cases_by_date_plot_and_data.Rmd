...
```{r}
usa_data = read_csv("../Statistical_Analysis/usafacts_infections.csv")
...
```
```{r}
county_info_data = read_csv("county_data_nona.csv")
...
```{r}
date_x = seq.Date(from = as.Date("2020/01/22",format = "%Y/%m/%d"), by = "day", length.out = 285)
...
```{r}
plot_ly(IL_daily_data, x = ~date) %>% 
  add_lines(y = ~deaths, name = 'cases', mode = 'lines') %>%
